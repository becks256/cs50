...
class MinesweeperAI:
    ...
    def add_knowledge(self, cell, count):
        ...
        neighbors = set()
        # for adjacent cells (i, j) to cell (x, y), i = x - 1, x, x + 1 and j = y - 1, y, y + 1
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                # check if the cell is in bounds and not the current cell itself
                if (i, j) != cell and 0 <= i < self.height and 0 <= j < self.width:
                    # add the cell to the neighbors set if it is not already a move made, safe or mine
                    if (
                        ...
                        neighbors.add((i, j))
                    # if the cell is a mine, decrement the count
    ...
    def update_knowledge_base(self):
        ...
        while changes:
            ...
            # collect known safes and mines from sentences
            for sentence in self.knowledge:
                all_safes.update(sentence.known_safes())
                all_mines.update(sentence.known_mines())

            # track all identified safes and mines in the parent class
            ...
            # take one sentence and compare it with all other sentences
            for sentence1 in self.knowledge:
                for sentence2 in self.knowledge:
                    # if we are not comparing the same sentence AND sentence1 is a subset of sentence2, then infer the cells
                    if sentence1 != sentence2 and sentence1.cells.issubset(
                        ...
                        # if the inferred sentence is not already captured AND its not in the knowledge base, add it
                        if (
                            inferred_sentence not in self.knowledge
                            and inferred_sentence not in inferred_sentences
                        ):
                            inferred_sentences.append(inferred_sentence)

        # finally, add the inferred sentences to the knowledge base and loop again to check for more inferences
    ...
    def make_random_move(self):
        ...
        return None  # No moves left to make
